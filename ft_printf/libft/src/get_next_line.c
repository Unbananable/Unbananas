...
/*   Updated: 2019/07/10 15:29:37 by anyahyao         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
** exeption permet de termine une lecture attention ne pas rappele gnl
** apres avoir mis l exeption
*/
...
int						get_next_line(int fd, char **l)
{
	...
	while (gnl->str[++i] && gnl->str[i] != '\n')
		(*l)[i] = gnl->str[i];
	(*l)[i] = '\0';
	if ((gnl->empty == 0 && i == 0) || gnl->exeption == 1)
		return (ft_decal_finish(&gnl, -1));
